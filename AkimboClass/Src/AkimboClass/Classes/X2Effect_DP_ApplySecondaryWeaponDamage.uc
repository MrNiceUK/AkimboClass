class X2Effect_DP_ApplySecondaryWeaponDamage extends X2Effect_ApplyWeaponDamage; 
//this is an exact copy of vanilla X2Effect_ApplyWeaponDamage effect, with one change that makes it apply damage from the weapon in secondary inventory slot
...
simulated function int CalculateDamageAmount(const out EffectAppliedData ApplyEffectParameters, out int ArmorMitigation, out int NewRupture, out int NewShred, out array<Name> AppliedDamageTypes, out int bAmmoIgnoresShields, out int bFullyImmune, out array<DamageModifierInfo> SpecialDamageMessages, optional XComGameState NewGameState)
{
	...
	// THIS IS THE ONLY PART THAT I CHANGED 
	if (kAbility != none) kSourceItem = kSourceUnit.GetItemInSlot(eInvSlot_SecondaryWeapon);		
	// IRIDAR
